...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    if (interaction.options.getSubcommand() === `start`) {
      let channeltoshow = interaction.options.getChannel("logchannel").id;
      let numwinner = interaction.options.getInteger("winners");
    ...
    if (interaction.options.getSubcommand() === `addticket`) {
      ...
      let length = guild.raffle.entries.length;
      let array2 = [];
      const user = interaction.options.getUser("user");
      let number = interaction.options.getInteger("tickets") || 1;
      ...
      collector.on("collect", async (int) => {
        if (int.user.id === interaction.user.id) {
          if (int.customId == "Y") {
            ...
            let numwinner = guild.raffle.numwinner;
            var winners = [];
