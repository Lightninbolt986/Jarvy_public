...
async execute(message, args, cmd, client, Discord){
    ...
    for (i = 0, j = Roles.length; i < j; i += chunk) {
      ...
      embed[`${i / 25}`] = new Discord.MessageEmbed()
        .setAuthor(`Roles in ${message.guild.name}`, `${message.guild.iconURL({ dynamic: true })}`)
    ...
    if (embedslist.length < 1) return message.channel.send('no roles in this server lol')
    pagination({
      ...
      fastSkip: true,
      button: [
          { name: "first", emoji: "<:bp_dlarrow:918356485065564181>", style: "PRIMARY" },
          { name: "previous", emoji: "<:bp_larrow:909009049104814100>", style: "PRIMARY" },
          { name: "next", emoji: "<:bp_rarrow:909009732243693619>", style: "PRIMARY" },
          { name: "last", emoji: "<:bp_drarrow:918356561804554310>", style: "PRIMARY" },
      ],
      time: 300000,
  })
}
