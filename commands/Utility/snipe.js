module.exports = {
  ...
  async execute(message, args, cmd, client, Discord) {
    snipes = client.snipes.filter(e => e.channel.id === message.channel.id).reverse();
    ...
    if (args[0] == 'group') {
      ...
      if (snipes.length < 6) {
        ...
        snipes.forEach(msg => {
          ...
          msgArr.push(`\n\━\━\━\━\━\━\━\━\━\━\━\━\━\━\━\━\━\━\n<:bp_srarrow:909689391994253313>**${msg.author}** \u2500 *${secondsToDhms(time)} ago*\nContent: ${msg.content}`)
        })
        desc = {}
        var k, j, temporary, chunk = 5,
          p = 1;
        for (k = 0, j = msgArr.length; k < j; k += chunk) {
          ...
          desc[`${p}`] = new Discord.MessageEmbed()
            ...
            .setTitle(`**Last ${temporary.length} messages deleted in __${message.channel.name}__**`)
      ...
      embed.setFooter(`Sniped by ${message.member.user.tag} | BTW you can add 'bulk' or 'group' for more snipes messages (if any)`)
...
const Discord = require('discord.js')
  ...
  if (i == 0) {
    ...
    if (i == 1) {
      ...
      embed.setTitle(`**Last ${i} messages deleted in __${message.channel.name}__**`)
