...
module.exports = {
  ...
  async execute(interaction, client) {
    let channel = interaction.options.getChannel("channel") || message.channel;
    let conf = false;
    ...
    let result = new embed().setColor("#2F3136");
    ...
    let filter1 = (i) => i.user.id === interaction.user.id;
    ...
    let pre = new embed().setColor("#2F3136").setDescription("None");
    ...
    let preview = await interaction.followUp({
      content: "**Preview:**",
      embeds: [pre],
      components: [but],
    });
...
async function handleModal(title, label, style) {
  const modal = new Modal().setCustomId("modal").setTitle("Enter ad");
  const adInput = new TextInputComponent()
    .setCustomId("adInput")
    .setLabel("Enter the ad including the role pings")
    .setStyle("PARAGRAPH")
    .setMaxLength(1800)
    .setRequired(true);
  modal.addComponents(new MessageActionRow().addComponents(adInput));
