module.exports = {
  ...
  async execute(message, args, cmd, client, Discord) {
    ...
    const fight = async function (options) {
      ...
      const Collector = await question.createMessageComponentCollector((fn) => fn, {
        time: 60000,
      });
              ...
              if (!checkHealth(player)) {
                ...
                if (msg.customId === id1) {
                  if (btn.id !== gameData[player].member.id) {
                    return msg.reply({ content: options.opponentsTurnMessage, ephemeral: true });
                  }
                  msg.deferUpdate()
                  let randNumb =
                  ...
                  if (gameData[player].health > 80) {
                    return msg.reply({content:options.highHealthMessage, ephemeral:true});
                  } else {
                  ...
                  if (gameData[player].health < 50) {
                    return msg.reply({ content: options.lowHealthMessage, ephemeral: true });
                  } else {
                    msg.deferUpdate()
